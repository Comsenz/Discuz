...
class UpdateUser
{
    ...
    public function handle(Dispatcher $events,UserRepository $repository)
    {
        ...
        foreach ($this->data['user'] as $k => $v) {
            ...
            if($user){
                $users=$repository->findOrFail($v,$this->actor);
                $this->events->dispatch(
                    new Deleting($user, $this->actor, ['$v'])
                );
                if (isset($this->data['operation']['adminid'])) {
                    $users->adminid = $this->data['operation']['adminid'];
                }
                if (isset($this->data['operation']['status'])) {
                    $users->status = $this->data['operation']['status'];
                }
                $users->save();
                // 调用钩子事件
