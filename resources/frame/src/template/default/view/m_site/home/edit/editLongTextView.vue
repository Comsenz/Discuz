<template>
  <div class="post-topic-box">
    ...
    <div class="post-longText-form" id="postForm" ref="postForm">
      <input
        type="text"
        placeholder="请输入标题"
        autofocus
      ...
      <textarea
        ...
        placeholder="请输入内容"
        v-model="content"
        @focus="showFacePanel = false; footMove = false; payMove=false; markMove=false; keyboard = false;"
      ></textarea>
    ...
    <markdown-toolbar
      for="textarea_id"
      class="markdownBox markdownFix"
      :class="{'markMove':markMove}"
      :style="{'overflow': 'hidden','width': (!isPhone && !isWeixin) ? '640px' : '100%','left': (!isPhone && !isWeixin) ? (viewportWidth - 640)/2+'px' : '0'}"
    >
      <md-bold>
      ...
      <md-unordered-list>
        <span class="icon iconfont icon-unordered-list"></span>
      </md-unordered-list>
      <md-ordered-list>
        <span class="icon iconfont icon-ordered-list"></span>
      </md-ordered-list>
    ...
    <van-cell
      ...
      is-link
      :value="payValue"
      :class="{'payMove':payMove}"
      class="paySetting"
      :style="{'overflow': 'hidden','width': (!isPhone && !isWeixin) ? '640px' : '100%','left': (!isPhone && !isWeixin) ? (viewportWidth - 640)/2+'px' : '0'}"
    />
    <footer
      class="post-topic-footer"
      id="post-topic-footer"
      :class="{'footMove':footMove}"
      :style="{'overflow': 'hidden','width': (!isPhone && !isWeixin) ? '640px' : '100%','left': (!isPhone && !isWeixin) ? (viewportWidth - 640)/2+'px' : '0'}"
    >
    ...
    <van-popup v-model="paySetShow" class="paySetShow" click-overlay="closePaySet">
      <div class="popTitBox">
      ...
      <a
        href="javascript:;"
        class="popSureBtn"
        :class="{ 'sureBtnCli': isCli, 'forbiddenCli': !isCli }"
        @click="isCli && paySetSure()"
      >确定</a>
    </van-popup>
    <Expression
      :faceData="faceData"
