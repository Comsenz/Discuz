<template>
  <div id="question">
    <div class="content">
      <div class="main">
        <a href="javascript:;" class="add-video" @click="choseVideo">
          <input type="file" accept="video/mp4" @change="changeVideo()" ref="vcVideo" />
          <div></div>
          <!-- <span>{{vcVideoName ? '点此重新上传视频' : '点击上传视频'}}</span> -->
          <span>{{vcVideoName ? vcVideoName : ''}}</span>
        </a>
        <div v-for="uploaderInfo in uploaderInfos" :key="uploaderInfo">
          <div v-if="uploaderInfo" class="upload-info">
    ...
    <div class="ques2-footer">
      <button
        :disabled="vcVideoName && disabledBtn === false ? false : true"
        @click="vcAddUpload"
      >提交</button>
    </div>
  </div>
    ...
    vcAddUpload() {
      ...
      uploader
        ...
        .then(function(doneResult) {
          ...
          API.saveSubAnswer({
            questions_id: self.state.matchInfo.sub[self.questionIndex].id,
            url: uploaderInfo.videoUrl
          })
            .then(res => {
              if (res.data.code === 200) {
                ...
                if (self.timeEnd) {
                  let configObj = {
                    ...
                    content:
                      "您可在5个工作日内，将视频发送至haixuan@dxy.cn。若超过5个工作日，则视为放弃。", // 提示框的内容
