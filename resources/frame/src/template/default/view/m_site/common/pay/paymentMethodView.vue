              ...
              >
                <div class="way-to-choose_cont-select_cell-left">
                  ...
                  <div class="way-to-choose_cont-select_cell-left-title">
                    <span>{{item.name}}</span>
                    <p
                      v-if="!walletStatus && item.name === '钱包'"
                      class="way-to-choose_cont-select_cell-left-title_description"
                      @click="payStatusClick"
                    >请设置钱包支付密码</p>
                    <p
                      v-else-if="descriptionShow && item.name === '钱包'"
                      class="way-to-choose_cont-select_cell-left-title_description"
                    >钱包余额不足，剩余{{balance}}元</p>
                  </div>
                </div>
                <van-radio
                  slot="right-icon"
                  :disabled="(descriptionShow || !walletStatus)&& item.name === '钱包'"
                  :name="index"
                />
              </div>
            </van-radio-group>
      ...
      <div class="pay-immediately-main">
        ...
        <div class="pay-immediately-main_cont">
          <van-cell
            title="支付方式"
            is-link
            @click="paySelectShow = !paySelectShow;payImmediatelyShow = !payImmediatelyShow"
          >
