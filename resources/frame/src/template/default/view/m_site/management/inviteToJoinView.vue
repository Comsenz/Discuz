          ...
          <div class="inviteBox">
            <table class="inviteTable">
              ...
              <tr v-for="(inviteLi, index) in inviteList" :key="index">
                ...
                <td v-if="inviteLi._data.status ==0">失效</td>
                <td v-if="inviteLi._data.status ==1">未使用</td>
                <td v-if="inviteLi._data.status ==2">已使用</td>
    ...
    >
      <div class="operaCho">
        ...
        <ul class="operaChoList" v-if="choiceShow">
          <li
            v-for="(item, index) in choList"
            :key="index"
            v-on:click.stop="setSelectVal(item)"
            class="operaChoLi"
          >{{item.attributes.name}} 邀请链接</li>
        </ul>
      </div>
